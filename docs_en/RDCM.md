...
Initial frame:
![RDCM_initial_frame](../docs/static/RDCM_Distance_First.png)


Final frame:
![RDCM_final_frame](../docs/static/RDCM_Distance_Last.png)

A certain intermediate output frame:
![RDCM_middle_frame](../docs/static/RDCM_Distance_Time_60000.0.png)

Difference between intermediate frame and its previous frame:
![RDCM_diff](../docs/static/RDCM_Distance_Time_60000.0-50000.0.png)

DIP will also calculate the average and standard deviation of all RDCM frames and output:

Average:
![RDCM_ave](../docs/static/RDCM_Distance_Average.png)

Standard deviation:
![RDCM_std](../docs/static/RDCM_Distance_Deviation.png)

If `calc_RMSD` is set to `yes`, the RMSD curve based on RDCM will be calculated:
![RDCM_RMSD](../docs/static/RDCM_RMSD.png)

If RMSD matrix output is set, the RMSD matrix will be output:
![RDCM_RMSD_Matrix](../docs/static/RDCM_RMSD_Matrix.png)

If `calc_RMSF` is set to `yes`, the RMSF curve based on RDCM will be calculated:
![RDCM_RMSF](../docs/static/RDCM_RMSF.png)

If `calc_DCCM` is set to `yes`, the DCCM matrix based on RDCM will be calculated:
![RDCM_DCCM](../docs/static/RDCM_DCCM.png)
...
![RDCM_Pearson_Time](../docs/static/RDCM_Pearson_Time.png)
...
![RDCM_Pearson_Time_pvalue](../docs/static/RDCM_Pearson_Time_p_Value.png)
...
![RDCM_Pearson_RMSD](../docs/static/RDCM_Pearson_Observe.png)

P-value matrix for Pearson correlation with RMSD:
![RDCM_Pearson_RMSD_pvalue](../docs/static/RDCM_Pearson_Observe_p_Value.png)

If `calc_PCA` is set to `yes`, PCA based on RDCM will be calculated, yielding scatter plots of three principal components:
![RDCM_PCA12](../docs/static/RDCM_PCA12.png)
![RDCM_PCA13](../docs/static/RDCM_PCA13.png)
![RDCM_PCA23](../docs/static/RDCM_PCA23.png)

Here we also set the clustering step, so there will be residue clustering and frame clustering plots:
![RDCM_Clustering_Frame](../docs/static/RDCM_Time_dendrogram.png)
![RDCM_Clustering_Residue](../docs/static/RDCM_Residues_dendrogram.png)

Below is the contact section, including contact occupancy matrix:
![RDCM_Contact_Occupancy](../docs/static/RDCM_Contact_Time_Occupancy.png)
...
![RDCM_Contact_Time](../docs/static/RDCM_contact_occupancy_curve.png)
...
![RDCM_Encounter_Time_First](../docs/static/RDCM_Encounter_FirstTime.png)

Last formation time matrix:
![RDCM_Encounter_Time_Last](../docs/static/RDCM_Encounter_LastTime.png)

Average formation time matrix:
![RDCM_Encounter_Time_Average](../docs/static/RDCM_Encounter_AvesTime.png)

Average time length matrix of Encounter:
![RDCM_Encounter_Time_Length](../docs/static/RDCM_Encounter_TimeLens.png)

Encounter time occupancy matrix:
![RDCM_Encounter_Occupancy](../docs/static/RDCM_Encounter_Time_Occupancy.png)

Similar local Encounter curve:
![RDCM_Encounter_Occupancy_Curve](../docs/static/RDCM_encounter_occupancy_curve.png)

Matrix of number of Encounter formations:
![RDCM_Encounter_Count](../docs/static/RDCM_Encounter_Count.png)
...
![RDCM_Encounter_DCCM](../docs/static/RDCM_Encounter_DCCM.png)
